...
namespace StoryRest.ArUco
{
    ...
    public class StoryRestApp : MonoBehaviour
    {
        ...
        /// 층마다 프로젝터 배정이 다르므로(1층 3대 / 2·3층 4대) 시작할 때 확인한다.
        /// </summary>
        void ValidateDisplayAssignment()
        {
            if (KeywordConfig == null || !KeywordConfig.enabled) return;
            if (KeywordConfig.displayIndexes == null || Config.sets == null) return;

            foreach (int wallDisplay in KeywordConfig.displayIndexes)
            {
                foreach (var set in Config.sets)
                {
                    if (set == null || set.displayIndex != wallDisplay) continue;

                    _problems.Add($"키워드 월과 ArUco 세트 '{set.name}' 이 같은 디스플레이({wallDisplay})를 씁니다. " +
                                  $"keywordwall.json 의 displayIndexes 를 확인하세요.");
                }
            }
        }
        ...
        void CreateKeywordWalls()
        {
            ...
            var displays = KeywordConfig.displayIndexes;
            ...
            {
                var go = new GameObject($"KeywordWall_{displays[i]}");
                go.transform.SetParent(transform, false);

                var wall = go.AddComponent<KeywordWall>();
                wall.Setup(KeywordConfig, words, displays[i], i);
                _walls.Add(wall);
            }

            Debug.Log($"[Keyword] 키워드 월 {_walls.Count}개를 만들었습니다 " +
                      $"(디스플레이 {string.Join(", ", displays)})");
        }

        /// <summary>
        /// 설정에 정의된 만큼 세트를 만든다. 층수로 개수를 추론하지 않는다 —
        /// 설치가 바뀌어도 코드를 고치지 않기 위함이다.
        /// </summary>
        void CreateSets()
        {
            if (Config.sets == null || Config.sets.Count == 0) return;

            int count = Config.sets.Count;
        ...
        /// <summary>
        /// 세트가 쓰는 디스플레이를 켠다. 활성화하지 않은 디스플레이에는 아무것도 출력되지 않는다.
        /// 에디터에는 디스플레이가 하나뿐이라 이 호출은 무시된다(→ editorPreview).
        /// </summary>
        void ActivateDisplays()
        {
            if (Config.sets != null)
            {
                foreach (var set in Config.sets)
                {
                    if (set != null) ActivateDisplay(set.displayIndex, $"세트 '{set.name}'");
                }
            }

            if (KeywordConfig != null && KeywordConfig.enabled && KeywordConfig.displayIndexes != null)
            {
                foreach (int index in KeywordConfig.displayIndexes) ActivateDisplay(index, "키워드 월");
            }
        }
        ...
        void LogSummary()
        {
            ...
            text.AppendLine($"[StoryRest] {Settings.floor}층 · 세트 {Config.sets?.Count ?? 0}개 " +
                            $"· 콘텐츠 {Content.Count}개");
